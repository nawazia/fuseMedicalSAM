...
@torch.no_grad()
def medsam_inference(medsam_model, img_embed, box_256, new_size, original_size, return_logits=False):
    """
...
def MedSAM_infer_npz_2D(img_npz_file, medsam_lite_model):
    ...
    if save_overlay:
        ...
        for i, mask in enumerate(gt_mask):
            color = colors[i]
            show_mask(mask.astype(np.uint8), ax[2], mask_color=color)
...
if __name__ == '__main__':
    ...
    for img_npz_file in tqdm(img_npz_files):
        ...
        if basename(img_npz_file).startswith('3D'):
            ...
            MedSAM_infer_npz_2D(img_npz_file, medsam_lite_model)
        end_time = time()
