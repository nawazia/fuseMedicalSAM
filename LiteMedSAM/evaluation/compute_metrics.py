...
def compute_multi_class_dsc(gt, seg):
    ...
    print(classes_in_gt, dsc)
    # remove nan values
    mean_dsc = np.mean([x for x in dsc if str(x) != 'nan'])
    return mean_dsc, dsc
...
def compute_multi_class_nsd(gt, seg, spacing, tolerance=2.0):
    ...
    for i in range(1, gt.max()+1):
        gt_i = gt == i
        seg_i = seg == i
        # print(seg_i.shape, gt_i.shape) # (256, 216, 1) (3, 256, 216, 1)
...
def compute_metrics(npz_name):
    ...
    gts = npz_gt['gts']
    labels = npz_gt['label_except_bk']
    compressed_mask = np.zeros(gts.shape[1:], dtype=np.uint8)  # Initialize with zeros
    for i in range(gts.shape[0]):
        compressed_mask[gts[i] == 1] += labels[i]
    gts = compressed_mask
    ...
    mean_dsc, dsc = compute_multi_class_dsc(gts, segs)
    # comupute nsd
    if compute_NSD:
        if mean_dsc > 0.2:
        # only compute nsd when dice > 0.2 because NSD is also low when dice is too low
            ...
            nsd = 0.0
    return npz_name, mean_dsc, nsd, dsc

if __name__ == '__main__':
    seg_metrics = OrderedDict()
    seg_metrics['case'] = []
    seg_metrics['mean_dsc'] = []
    seg_metrics['dsc_0'] = []
    seg_metrics['dsc_1'] = []
    seg_metrics['dsc_2'] = []
    if compute_NSD:
    ...
    with mp.Pool(num_workers) as pool:
        with tqdm(total=len(npz_names)) as pbar:
            for i, (npz_name, mean_dsc, nsd, dsc) in enumerate(pool.imap_unordered(compute_metrics, npz_names)):
                seg_metrics['case'].append(npz_name)
                seg_metrics['mean_dsc'].append(np.round(mean_dsc, 4))
                for idx, i in enumerate(dsc):
                    seg_metrics[f'dsc_{idx}'].append(i)
                if compute_NSD:
                    seg_metrics['nsd'].append(np.round(nsd, 4))
                pbar.update()
    print(np.mean([x for x in seg_metrics['dsc_0'] if str(x) != 'nan'])*100, np.std([x for x in seg_metrics['dsc_0'] if str(x) != 'nan'])*100)
    print(np.mean([x for x in seg_metrics['dsc_1'] if str(x) != 'nan'])*100, np.std([x for x in seg_metrics['dsc_1'] if str(x) != 'nan'])*100)
    print(np.mean([x for x in seg_metrics['dsc_2'] if str(x) != 'nan'])*100, np.std([x for x in seg_metrics['dsc_2'] if str(x) != 'nan'])*100)
