from segment_anything import sam_model_registry_sam_med2d
...
def prompt_and_decoder(args, batched_input, ddp_model, image_embeddings):
    ...
    with torch.no_grad():
        ...
        print(f"Low res masks shape: {low_res_masks.shape}, IOU predictions shape: {iou_predictions.shape}")
...
def main(args):
    ...
    model = sam_model_registry_sam_med2d[args.model_type](args.image_size, args.sam_checkpoint, args.encoder_adapter).to(args.device) 
    ...
    for i, batched_input in enumerate(test_pbar):
        batched_input = to_device(batched_input, args.device)       # iterate through batched_input which includes all prompts (unused are None) + image 
        ...
        # stats of image data
        print("Image size:", batched_input["image"].shape, "mean:", batched_input["image"].mean().item(), "std:", batched_input["image"].std().item(), "min:", batched_input["image"].min().item(), "max:", batched_input["image"].max().item())
        print("boxes:", batched_input["boxes"], "shape:", batched_input["boxes"].shape)
        ...
        # cv2.imwrite(f"img.tif", (batched_input["image"].squeeze()[0].cpu().numpy()).astype(np.float32))
        with torch.no_grad():
        ...
        # masks = torch.sigmoid(masks)  # convert logits to probabilities
        cv2.imwrite(f"{img_name}_mask_logits.tif", (masks.squeeze().cpu().numpy()).astype(np.float32))
        if args.save_pred:
