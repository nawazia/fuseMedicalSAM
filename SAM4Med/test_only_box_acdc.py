...
if __name__ == "__main__":
    ...
    sam_checkpoint = f"model/medsam_box_best_vitb.pth"
    ...
    save_base = f"work_dir_{size}/{task}/pred_only_box"
    save_prompt = f"work_dir_{size}/{task}/prompts_only_box"
    ...
    for ori_path in tqdm(glob.glob(f"data/precompute_vit_b/test/acdc/*.npz")):
        ...
        if not os.path.exists(os.path.join(save_base, ori_path.split('/')[-1].replace('png', 'npy'))):
            ...
            np.save(os.path.join(save_base, ori_path.split('/')[-1].replace('npz', 'npy')), masks)
        else:
        ...
        if not os.path.exists(os.path.join(save_base, ori_path.split('/')[-1])):
            cv2.imwrite(os.path.join(save_base, ori_path.split('/')[-1].split(".")[0] + ".png"), current_method_res.astype(np.uint8))
