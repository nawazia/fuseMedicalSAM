...
join = os.path.join
import torch
from segment_anything import sam_model_registry_medsam
...
device = args.device
medsam_model = sam_model_registry_medsam["vit_b"](checkpoint=args.checkpoint)
...
print(box_1024)
cv2.imwrite(f"img_medsam.tif", (img_1024_tensor.float().squeeze()[0].cpu().numpy()).astype(np.float32))
with torch.no_grad():
