...
def train_sam(args, net: nn.Module, optimizer, train_loader,
    ...
    epoch_loss = 0
    GPUdevice = torch.device('mps:' + str(args.gpu_device))
    ...
    with tqdm(total=len(train_loader), desc=f'Epoch {epoch}', unit='img') as pbar:
        for pack in train_loader:
            ...
            # plt.figure()
            # plt.imshow(masks.squeeze().cpu().detach().numpy()[0], cmap="gray")
            # plt.figure()
            # plt.imshow(masks.squeeze().cpu().detach().numpy()[1], cmap="gray")
            # plt.show()
            # masks_1 = torch.select(masks, dim=1, index=0).unsqueeze(0)
