...
args = cfg.parse_args()
device = torch.device('mps', args.gpu_device)
...
class DiceCoeff(Function):
    ...
    def forward(self, input, target):
        ...
        eps = 0.0001
        # eps = torch.as_tensor(0.0001, dtype=torch.float32).to(device)
...
def dice_coeff(input: torch.Tensor, target):
    """Dice coeff for batches"""
    if input.is_cuda or input.is_mps:
...
def cppn(args, size, img = None, seg = None, batch=None, num_output_channels=1, num_hidden_channels=128, num_layers=8,
         activation_fn=CompositeActivation, normalize=False, device = "mps:0"):
...
def get_siren(args):
    wrapper = get_network(args, 'siren', use_gpu=args.gpu, gpu_device=torch.device('mps', args.gpu_device), distribution = args.distributed)
    '''load init weights'''
    ...
    vae = get_network(args, 'vae', use_gpu=args.gpu, gpu_device=torch.device('mps', args.gpu_device), distribution = args.distributed)
